...
# What to do next
- Dig into the planktivore species, look back to what is happening to trailing/leading/COG dynamics over time -- big implications for ecosystem/structure and food web dynamics as it seems like these are key forage species
- "Flat" line for piscivores could be because some are increasing, some are decreasing
- New model: Drop SOE 24 and then add species as FE, gets you a slope for each species -- `model <- glmmTMB(dist_km ~ delta_year_day + est_year*species + (1|year_fac), family = Gamma(link = "log"), data = lme)` and then change to Gamma (log link) and just use dist_km as response (don't use log transformed). Check the formula as I just did that off the top of my head for family/link :)
- Extract slope/CI for each species, generate plot, and color by SOE.24 grouping
